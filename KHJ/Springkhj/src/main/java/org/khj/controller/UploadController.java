...
public class UploadController {
	...
	private boolean checkImage(File file) {
		try {
			...
			System.out.println("aaaaa="+contentType.startsWith("image"));
	...
	public ResponseEntity<byte[]> getFile(String fileName) { // getFile()은 문자열로 파일의 경로가 포함된 fileName을 매개변수 받고 byte[](이진수)를 전송
		...
		File file = new File("D:\\KHJ\\upload\\"+fileName);
	...
	public ResponseEntity<Resource> downloadFile(String fileName){
		...
		Resource resource = new FileSystemResource("D:\\KHJ\\upload\\" + fileName);
